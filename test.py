    ...
    while i > 0:
        ...
        for j, instr in enumerate(list_of_inst_obj):
            ...
            if instr.status == 'IF' and not pipe_obj.fetch_busy[0]:
                ...
                #         print(pipe_obj.calculated_reg.items())
                #         if pipe_obj.calculated_reg['R1'] != pipe_obj.calculated_reg['R3']:
                #             cont_inst = pipe_obj.inst[0][pipe_obj.inst[1]['GG']:]
                #             for inst in cont_inst:
                #                 list_of_inst_obj.append(Instructions(inst))
